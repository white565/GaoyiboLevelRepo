...
Shader "TidalFlask/Top Projection Gradient URP"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float2 texCoord197 = IN.ase_texcoord7.xy * float2( 1,1 ) + float2( 0,0 );
				#ifdef _GRADIENTBYWORLDORUV1_ON
				float staticSwitch199 = WorldPosition.y;
				#else
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float4 lerpResult62 = lerp( float4( desaturateVar131 , 0.0 ) , ( _TopTextureTint * tex2D( _TopTexture, (appendResult113*_TopTextureTiling + 0.0) ) ) , saturate( ( pow( saturate( ( simplePerlin2D148 + temp_output_96_0 ) ) , _CoverageFalloff ) + pow( saturate( ( simplePerlin2D148 + ( temp_output_96_0 * 0.85 ) ) ) , _CoverageFalloff ) ) ));
				float2 texCoord197 = IN.ase_texcoord2.xy * float2( 1,1 ) + float2( 0,0 );
				#ifdef _GRADIENTBYWORLDORUV1_ON
				float staticSwitch199 = WorldPosition.y;
				#else
				float staticSwitch199 = texCoord197.y;
...
/*ASEBEGIN
Version=18912
767;312;1744;1156;3273.463;764.7147;2.819453;True;False
...
ASEEND*/
//CHKSM=CA63EA6A12BF65C7037B9C34472D47C2528D05BB